    ...
    monaco.languages.registerCompletionItemProvider(DSL_LANGUAGE_ID, {
        provideCompletionItems(model, position) {
            ...
            const snippets: Array<{ label: string; insert: string; doc: string }> = [
                { label: 'class', insert: 'class ${1:ClassName}\n  + ${2:field}: ${3:string}\n  + ${4:method}(${5:param}:${6:string}): ${7:void}', doc: 'Class declaration' },
                { label: 'class extends', insert: 'class ${1:ClassName} extends ${2:BaseClass}\n  + ${3:field}: ${4:string}', doc: 'Class with extends' },
                { label: 'class implements', insert: 'class ${1:ClassName} implements ${2:IService}, ${3:ILogging}\n  + ${4:method}(): ${5:void}', doc: 'Class with implements' },
                { label: 'class full', insert: 'class ${1:ClassName} extends ${2:BaseClass} implements ${3:IService}, ${4:ILogging}\n  + ${5:field}: ${6:string}\n  + ${7:method}(${8:param}:${9:string}): ${10:void}', doc: 'Class with extends and implements' },
                { label: 'interface', insert: 'interface ${1:IName}\n  + ${2:method}(${3:param}:${4:string}): ${5:void}', doc: 'Interface declaration' },
                { label: 'struct', insert: 'struct ${1:Name}\n  + ${2:field}: ${3:int}', doc: 'Struct declaration' },
                { label: 'abstract class', insert: 'abstract class ${1:Name}\n  + a ${2:method}(${3:param}:${4:string}): ${5:void}', doc: 'Abstract class declaration' },
                { label: 'extends', insert: 'extends ${1:ParentClass}', doc: 'Extends line inside class block' },
                { label: 'implements', insert: 'implements ${1:IService}, ${2:ILogging}', doc: 'Implements line inside class block' },
                { label: 'member', insert: '+ ${1:field}: ${2:string}', doc: 'Member declaration' },
                { label: 'static member', insert: '+ s ${1:field}: ${2:string}', doc: 'Static member declaration' },
                { label: 'abstract member', insert: '+ a ${1:field}: ${2:string}', doc: 'Abstract member declaration supported by parser' },
                { label: 'operation', insert: '+ ${1:method}(${2:param}:${3:string}): ${4:void}', doc: 'Operation declaration' },
                { label: 'static operation', insert: '+ s ${1:method}(${2:param}:${3:string}): ${4:void}', doc: 'Static operation declaration' },
                { label: 'abstract operation', insert: '+ a ${1:method}(${2:param}:${3:string}): ${4:void}', doc: 'Abstract operation declaration' },
                { label: 'needs', insert: 'needs ${1:owner }"${2:reason}"', doc: 'Member needs metadata on the next line' },
                { label: 'alias', insert: 'alias "${1:displayName}" as "${2:identifierName}"', doc: 'Alias declaration' },
                { label: 'endpoint', insert: 'endpoint ${1|GET,POST,PUT,PATCH,DELETE,HEAD,OPTIONS|} ${2:/api/resource}', doc: 'Endpoint declaration' },
                { label: '->', insert: '${1:Source} -> ${2:Target}', doc: 'Association relation' },
                { label: '-> labeled', insert: '${1:Source} -> ${2:Target} :${3:uses}', doc: 'Association with label' },
                { label: '-> multiplicity', insert: '${1:Source} -> ${2:Target} ${3:1} ${4:*}', doc: 'Association with multiplicity' },
                { label: '-/>', insert: '${1:Source} -/> ${2:Target}', doc: 'Dependency relation' },
                { label: '+>', insert: '${1:Factory} +> ${2:Product}', doc: 'Instantiation relation' },
                { label: 'o>', insert: '${1:Whole} o> ${2:Part}', doc: 'Aggregation relation' },
                { label: '*>', insert: '${1:Whole} *> ${2:Part}', doc: 'Composition relation' },
                { label: '>|', insert: '${1:Child} >| ${2:Parent}', doc: 'Generalization relation' },
                { label: '>/', insert: '${1:Class} >/ ${2:Interface}', doc: 'Realization relation' },
                { label: 'Scenario', insert: 'Scenario: ${1:Scenario Name}\n  Given ${2:precondition}\n  When ${3:action}\n  Then ${4:expected result}', doc: 'Gherkin scenario' },
                { label: 'Scenario src', insert: 'Scenario: ${1:Scenario Name} src: ${2:REQ-001} ${3:https://example.com/spec#1}\n  Given ${4:precondition}\n  When ${5:action}\n  Then ${6:expected result}', doc: 'Scenario with src traceability' },
                { label: 'Given', insert: 'Given ${1:precondition}', doc: 'Gherkin: Given' },
                { label: 'When', insert: 'When ${1:action}', doc: 'Gherkin: When' },
                { label: 'Then', insert: 'Then ${1:expected result}', doc: 'Gherkin: Then' },
                { label: 'And', insert: 'And ${1:additional condition}', doc: 'Gherkin: And' },
                { label: 'But', insert: 'But ${1:exception condition}', doc: 'Gherkin: But' },
                { label: 'How', insert: 'How ${1:implementation note}\n  "${2:step one}"\n  "${3:step two}"', doc: 'Gherkin: How with detail lines' },
                { label: 'Why', insert: 'Why ${1:rationale}', doc: 'Gherkin: Why' },
