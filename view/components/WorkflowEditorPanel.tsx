...
export type NodeType =
    ...
    | 'given' | 'when' | 'then' | 'how'
    | 'foreach' | 'forrange' | 'switch' | 'break' | 'continue' | 'return'
...
function nodeSize(type: NodeType): { w: number; h: number } {
    ...
    if (type === 'return') return { w: 130, h: 38 }
    return { w: 140, h: 40 }
}
...
export function convertAstToWorkflow(ast?: FlowAst): WFWorkflow {
    ...
    // appendSeq
    //   loopExitId: 現在のスコープで break が飛ぶべき exit ノードのID（ループ外は null）
    //   loopNodeId: 現在のスコープで continue が戻るべきループノードのID（ループ外は null）
    const appendSeq = (
        fromId: string,
        stmts: any[],
        ...
        for (const s of stmts) {
            ...
            if (s.type === 'if') {
                ...
                if (thenStmts.length > 0) {
                    globalY = savedY
                    const r = appendSeq(decId, thenStmts, 'true', baseX + LANE_W, loopExitId, loopNodeId)
                ...
                if (elseStmts.length > 0) {
                    globalY = savedY
                    const r = appendSeq(decId, elseStmts, 'false', baseX - LANE_W, loopExitId, loopNodeId)
            ...
            if (s.type === 'while') {
                ...
                if (bodyStmts.length > 0) {
                    ...
                    const r = appendSeq(loopId, bodyStmts, 'true', baseX + BODY_OFFSET, exitId, loopId)
            ...
            if (s.type === 'forEach') {
                ...
                const bodyStmts: any[] = Array.isArray(s.body) ? s.body : []
                if (bodyStmts.length > 0) {
                    const r = appendSeq(forId, bodyStmts, 'body', baseX + BODY_OFFSET, exitId, forId)
            ...
            if (s.type === 'switch') {
                ...
                cases.forEach((c: any, idx: number) => {
                    ...
                    globalY = savedY
                    if (cs.length > 0) {
                        caseResults.push(appendSeq(swId, cs, String(c?.value ?? `case${idx}`), cx, loopExitId, loopNodeId))
                    } else {
                ...
                if (hasDef) {
                    ...
                    caseResults.push(appendSeq(swId, s.default, 'default', dx, loopExitId, loopNodeId))
                    if (globalY > maxY) maxY = globalY
                } else {
            ...
            // ── 末端ノード ───────────────────────────────────
            const nt: NodeType = s.type === 'break' ? 'break' : s.type === 'continue' ? 'continue' : s.type === 'return' ? 'return' : 'process'
            ...
            if (s.type === 'continue') {
                // continue → ループノード自身へループバックエッジを張る
                if (loopNodeId) addEdge(id, loopNodeId, undefined, true)
...
export function convertToAst(wf: WFWorkflow) {
    ...
    function walk(id: string | undefined, stop: string | null = null): any[] {
        ...
        if (n.type === 'return') return [{ type: 'return', value: n.label.replace(/^return\s*/, '') || undefined }]
...
const STYLE: Record<NodeType, { fill: string; stroke: string; text: string }> = {
    ...
    return: { fill: '#1a1a2e', stroke: '#f59e0b', text: '#fef3c7' },
}
...
function arrowRightPts(w: number, h: number): string {
    // 右向き矢印五角形: 左辺が矩形、右端が三角形の突起
    const cx = w / 2, cy = h / 2, tip = w * 0.28
    return [[-cx, -cy], [cx - tip, -cy], [cx, 0], [cx - tip, cy], [-cx, cy]].map(p => p.join(',')).join(' ')
}
...
function NodeShape({ node, isSelected, onPointerDown, onHandlePointerDown, onDoubleClick, onContextMenu }: NodeShapeProps) {
    ...
    if (node.type === 'return') return wrap(<>
        <polygon points={arrowRightPts(w, bH)} {...bp} />
        <text x={-w / 2 + 18} y={1} textAnchor="middle" dominantBaseline="central" fontSize={10} fill={st.stroke} pointerEvents="none">↩</text>
        <text x={12} y={1} textAnchor="middle" dominantBaseline="central" fontSize={10} fill={st.text} fontWeight="600" fontFamily='"Cascadia Code",monospace' pointerEvents="none">{node.label.length > 14 ? node.label.slice(0, 13) + '…' : node.label}</text>
    </>)
...
const FLOW_NODE_TYPES: NodeType[] = ['foreach', 'forrange', 'switch', 'break', 'continue', 'return']
const NODE_COL: Record<NodeType, string> = { start: '#4ade80', end: '#f87171', process: '#94a3b8', decision: '#c084fc', loop: '#38bdf8', call: '#fb923c', given: '#22c55e', when: '#a855f7', then: '#60a5fa', how: '#1d4ed8', foreach: '#2dd4bf', forrange: '#14b8a6', switch: '#f59e0b', break: '#f87171', continue: '#a78bfa', return: '#f59e0b' }
const FLOW_NODE_LABEL: Record<string, string> = { foreach: 'ForEach', forrange: 'ForRange', switch: 'Switch', break: 'Break', continue: 'Continue', return: 'Return' }
