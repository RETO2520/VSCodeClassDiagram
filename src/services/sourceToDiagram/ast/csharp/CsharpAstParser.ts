...
export class CsharpAstParser implements IAstParser {
    ...
    private async initParser(): Promise<boolean> {
        ...
        try {
            ...
            let wasmPath: string;
            try {
                // tree-sitter-c-sharpパッケージ内のwasmファイルを解決
                wasmPath = require.resolve('tree-sitter-c-sharp/tree-sitter-c_sharp.wasm');
                this.logger.info(`Found wasm file at: ${wasmPath}`);
            } catch (e) {
                // フォールバック: 相対パスでの解決
                wasmPath = path.join(__dirname, '../../../../../node_modules/tree-sitter-c-sharp/tree-sitter-c_sharp.wasm');
    ...
    private extractClassInfo(node: Node, uri: vscode.Uri): ClassInfo {
        ...
        if (baseList) {
            ...
            if (types.length > 0) {
                // 簡易的に最初のものをbaseClassとし、それ以降をinterfacesとする
                classInfo.baseClass = types[0].text;
